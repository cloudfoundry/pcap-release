...
if [ -n "$FOCUS" ]; then
  docker run --privileged -v "$REPO_DIR":/repo -e REPO_ROOT=/repo -e FOCUS="$FOCUS" pcap-release-testflight bash -c "cd /repo/ci/scripts && ./acceptance-tests ; sleep infinity"
else
  docker run --rm --privileged -v "$REPO_DIR":/repo -e REPO_ROOT=/repo pcap-release-testflight bash -c "cd /repo/ci/scripts && ./acceptance-tests"
fi

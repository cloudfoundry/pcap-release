...
def run_vendor_package():
    ...
    # process bosh vendor-package response (expected response/stdout: "2022/06/12 11:00:35 Successfully uploaded file to https://s3.amazonaws.com/cfi-boshrelease-blobstore/45cd0a6e-3000-45cb-6199-d2590c22b583")
    if response == "":
...
def check_for_conflicting_bump_branch(repo):
    # the bump branch should not exist after a successful previous run/merge. If there is an existing branch, we risk including other changes in PR, so we abort instead.
...
def create_pr(current_version, new_version, blob_link):
    ...
    # add changed files
    LOCAL_GIT.add(GOLANG_RELEASE_INDEX_YML_PATH)
    ...
    print("Requesting review from CFN team...")
...
def update_git_content(file_path, file_content, latest_version):
    ...
    if file_path == "go.mod":
        ...
        # Regex substition for version number in Yaml File, e.g.
        # env:
        #   GOLANG_VERSION: 1.20
